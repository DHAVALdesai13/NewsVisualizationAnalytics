...
d3.select("#selectButton").on("change", function(d) {
  ...
  dhav.update(selectedOption, 2016, 2019 )
})



function annie(selectdiv3){
  var margin = {top: 20, right: 20, bottom: 30, left: 40},
    width = 500 - margin.left - margin.right,
    height = 400 - margin.top - margin.bottom;

  var svg = d3.select(selectdiv3).append("svg")
...
var z = d3.scaleOrdinal()
    .range(["#ca0020","#f4a582","#d5d5d5","#92c5de","#0571b0", "#d0743c", "#ff8c00"]);

d3.csv("./js/d3/data/bar_real.csv", function(d, i, columns) {
...
change(2016,2019);
function change(startYear,endYear){
  ...
  console.log(datum)
  x0.domain(datum.map(function(d) { return d.Publisher; }));
  x1.domain(keys).rangeRound([0, x0.bandwidth()]);
  y.domain([0, d3.max(datum, function(d) { return d3.max(keys, function(key) { return d[key]; }); })]).nice();
 ...
 g.append("g")
    ...
    .enter().append("g")
    .attr("class","bar")
    .attr("transform", function(d) { return "translate(" + x0(d.Publisher) + ",0)"; })
    .selectAll("rect")
  ...
  function update(d) {  
    ...
    y.domain([0, d3.max(datum, function(d) { return d3.max(keys, function(key) { if (filtered.indexOf(key) == -1) return d[key]; }); })]).nice();
...
var svg = d3v3.select(selectdiv1).append("svg")
  .attr("width", width + margin.left + margin.right)
...
d3.tsv("./js/d3/data/timedata_real.tsv", function(error, data) { 
  data = data.filter(function(d){return d.Publisher=="Reuters"});
color.domain(d3.keys(data[0]).filter(function(key) { // Set the domain of the color ordinal scale to be all the csv headers except "date", matching a color to an issue
  return key !== "date" && key != "Publisher"; 
}));
...
var categories = color.domain().map(function(name) { // Nest the data into an array of objects with new keys
  ...
  return {
    ...
    visible: (name === "CRIME" ? true : false) // "visible": all false except for economy which is true.
  };
});
...
function dhav(selectdiv2){
    ...
    ////////////////////////////////////////////////////////////// 
      ...
      var margin = {top: 35, right: 100, bottom: 100, left: 150},
...
d3v3.csv("./js/d3/data/radar_real.csv", function(d) {
temp = [];
...
function(error, rows){
 console.log(rows); 
    ...
    var data = rows.filter(function(d){return d.name=="TECH"});
 ...
 var allGroup = d3v3.map(rows, function(d){return(d.name)}).keys()
 ...
 function update(selectedGroup,startYear, endYear){
    ...
    console.log("inside",startYear,endYear)
  
    console.log("dhaval",data)
...
var cfg = {
 ...
 margin: {top: 10, right: 10, bottom: 10, left: 20}, //The margins of the SVG
